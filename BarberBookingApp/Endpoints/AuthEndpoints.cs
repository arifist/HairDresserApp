...
public static class AuthEndpoints
{
    public static void MapAuthEndpoints(this WebApplication app)
    {
        ...
        group.MapPost("/verify-otp", async (HttpContext http, JsonDataStore store, IOtpService otpService) =>
        {
            ...
            if (string.IsNullOrWhiteSpace(returnUrl)) returnUrl = "/randevularim";

            var verified = await otpService.VerifyCodeAsync(phoneNumber, code);
            if (!verified)
                return Results.Redirect($"/giris?error=invalid-code&returnUrl={Uri.EscapeDataString(returnUrl)}");

            var customer = await store.FindCustomerByPhoneAsync(phoneNumber);
            if (customer is null)
            {
                ...
                await store.AddCustomerAsync(customer);
            }
        ...
        group.MapPost("/admin-login", async (HttpContext http, JsonDataStore store) =>
        {
            var form = http.Request.Form;
            var username = form["username"].ToString().Trim();
            var password = form["password"].ToString();
            var returnUrl = form["returnUrl"].ToString();
            if (string.IsNullOrWhiteSpace(returnUrl)) returnUrl = "/admin/panel";

            var admin = await store.FindAdminByUsernameAsync(username);
            if (admin is null) return Results.Redirect("/admin/giris?error=1");

            var hasher = new PasswordHasher<Admin>();
            var verifyResult = hasher.VerifyHashedPassword(admin, admin.PasswordHash, password);
            if (verifyResult == PasswordVerificationResult.Failed) return Results.Redirect("/admin/giris?error=1");
