...
### Adımlar
1. `BarberBookingApp/appsettings.json` içindeki `ConnectionStrings:DefaultConnection` değerini kendi MSSQL ortamınıza göre düzenleyin. Varsayılan değer LocalDB kullanır:
   ```
   Server=(localdb)\MSSQLLocalDB;Database=BarberBookingAppDb;Trusted_Connection=True;TrustServerCertificate=True;
   ```
...
## Geliştirme Notları
- Veritabanı migration'ları `BarberBookingApp/Migrations` klasöründedir. Modelde değişiklik yaparsanız:
  ```
